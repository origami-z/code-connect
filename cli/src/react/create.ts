...
export function generateExpressionFromIntrinsic({ kind, args }: Intrinsic): string | never {
  ...
  if (kind === IntrinsicKind.Boolean) {
    return `figma.boolean("${args.figmaPropName}"${
      args.valueMapping ? `, ${generateExpressionValueMapping(args.valueMapping)}` : ''
    })`
  }
  if (kind === IntrinsicKind.Enum) {
    return `figma.enum("${args.figmaPropName}"${
      args.valueMapping ? `, ${generateExpressionValueMapping(args.valueMapping)}` : ''
    })`
  }
  ...
  if (kind === IntrinsicKind.Children) {
    return `figma.children(${
      args.layers.length > 1
        ? `[${args.layers.map((layer) => `"${layer}"`).join(', ')}]`
        : `"${args.layers[0]}"`
    })`
  }
 ...
 * code example you'd like to see in Figma`
  ...
  const codeConnectCodePart = `
figma.connect(${importName}, "${figmaNodeUrl}", {
  props: ${
    propMapping ? generatePropsFromMapping(component, propMapping) : generateProps(component)
  },
...
`
  ...
  if (fs.existsSync(filePath)) {
    fs.appendFileSync(filePath, codeConnectCodePart)

    return {
      createdFiles: [{ filePath }],
      messages: [
        { message: `File ${filePath} already exists, append to existing content`, level: 'WARN' },
      ],
    }
  }
