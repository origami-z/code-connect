  ...
  try {
    ...
    const response = process.env.CODE_CONNECT_MOCK_CREATE_API_RESPONSE
      ? {
          ...
          data: JSON.parse(
            fs.readFileSync(
              process.env.CODE_CONNECT_MOCK_CREATE_API_RESPONSE,
              "utf-8"
            )
    ...
    if (response.response.status === 200) {
      ...
      const component = findComponentsWithinNode(
      ...
      logger.debug(
        "Failed to get node information from Figma with Body:",
        response.data
      );
    }
  } catch (err) {
    if (isFetchError(err)) {
      if (err.response) {
        logger.error(
          `Failed to get node data from Figma (${err.response.status}): ${
            err.response.status
